...
//var toDosDone: boolean[] =    [true    , false   , false];
...
function drawListToDOM(): void {
    ...
    for (let index: number = 0; index < todoObjects.length; index++) {
         ...
         */
        todo.innerHTML =  "<span class='check " + todoObjects[index].toDosDone + "'><i class='fas fa-check'></i></span>"
                            + todoObjects[index].todosText +
...
function updateCounter(): void {
    ...
    while (i < todoObjects.length) { 
        if (todoObjects[i].toDosDone == true) {
            checkmark++;
        }
        i++;
    }

    counterDoneDOMElement.innerHTML = checkmark + " done"; // alle mit toDosDone = true

    counterDOMElement.innerHTML = todoObjects.length + " total";

    counterTodoDOMElement.innerHTML = todoObjects.length - checkmark + " open"; //alle mit todosChecked = false
}
...
function toggleCheckState(index: number): void {
    ...
    todoObjects[index].toDosDone = ! todoObjects[index].toDosDone;
...
// Markus und Marie haben mir bei der Lösung sehr geholfen